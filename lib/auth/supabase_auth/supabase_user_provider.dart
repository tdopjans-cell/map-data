...
class Way2GoXSupabaseUser extends BaseAuthUser {
  Way2GoXSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> way2GoXSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = Way2GoXSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
